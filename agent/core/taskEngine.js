...
const overwriteFile = (filePath) => {
  ...
  while (remaining > 0) {
    ...
    fs.writeSync(fd, buffer, 0, size, position);
...
export const startWipeTask = async (socket, job) => {
  const {
    userId,
    commandId,
    storagePath,
  ...
  console.log("================================");
  console.log("START WIPE RECEIVED");
  console.log(job);
  console.log("================================");
  ...
  try {
    emitProgress(5, "Scanning storage...");
    ...
    const files = getFiles(storagePath);
    ...
    if (total === 0) {
      emitProgress(100, "No files found");
    ...
    removeEmptyDirectories(storagePath);
    const verificationHash = crypto
  .createHash("sha256")
  .update(commandId + Date.now())
  .digest("hex");
    socket.emit("wipe-complete", {
      commandId,
      userId,
  ...
  verificationHash,
  verificationEvidenceHash: verificationHash,
    });

    console.log("Wipe completed:", commandId);
...
export const cancelWipeTask = (commandId) => {
  cancelJob(commandId);
};